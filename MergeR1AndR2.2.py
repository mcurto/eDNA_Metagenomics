...
def findRightFile(fastaDir, sampleName, pattern):
    for f in os.listdir(fastaDir):
        if f.startswith(sampleName) and f.endswith(pattern):
            print(f)
            return fastaDir + f



def getFastaLengths(fastaDirList, sampleName, F_sufix, R_sufix):
    result = {}
    for fastaDir in fastaDirList:
        fasta1 = findRightFile(fastaDir, sampleName, F_sufix)
        fasta2 = findRightFile(fastaDir, sampleName, R_sufix)
        with open(fasta1) as f1:
            print("process paired fasta1")
            for l1 in f1:
                if l1.startswith(">"):
                    name = l1[1:].split(" ")[0]
                    result[name] = 0
                else:
                    result[name] = result.get(name, 0) + len(l1.rstrip("\n"))
        with open(fasta2) as f2:
            print("process paired fasta2")
            for l2 in f2:
                if l2.startswith(">"):
                    name = l2[1:].split(" ")[0]
                else:
                    result[name] = result.get(name, 0) + len(l2.rstrip("\n"))
...
def recalculateData(data1, data2):
    ...
    newID = round(((nrMM1 + nrMM2) / newalignement) * 100, 3)
...
for b in os.listdir(lineageR1):
    ...
    sampleName = b.split("_")[0]
    blastoutF2 = lineageR2 + b.replace("_1", "_2")
    ...
    paired_lengths = getFastaLengths(fastaDirs, sampleName, F_sufix, R_sufix)
